...
start_stack() {
    ...
    echo -e "${WHITE}  Dashboard:   http://${SERVER_IP}${NC}"
    echo -e "${WHITE}  pgAdmin:     http://${SERVER_IP}/pgadmin${NC}"
    echo -e "${WHITE}  RabbitMQ:    http://${SERVER_IP}/rabbitmq${NC}"
    echo -e "${WHITE}  MinIO:       http://${SERVER_IP}/minio${NC}"
    echo -e "${WHITE}  Keycloak:    http://${SERVER_IP}/keycloak${NC}"
    echo -e "\n${YELLOW}Direct port access:${NC}"
    echo -e "${GRAY}  pgAdmin:     http://${SERVER_IP}:5050${NC}"
    echo -e "${GRAY}  RabbitMQ:    http://${SERVER_IP}:15672${NC}"
    echo -e "${GRAY}  MinIO:       http://${SERVER_IP}:9001${NC}"
    echo -e "${GRAY}  Keycloak:    http://${SERVER_IP}:8080${NC}"
    echo -e "\n${YELLOW}Database connections:${NC}"
    echo -e "${GRAY}  PostgreSQL:  ${SERVER_IP}:5432${NC}"
